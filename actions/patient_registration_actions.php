...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query .= "patient_name,surname,date_of_birth,gender,weight,height,respiratory_rate,blood_sugar,bmi";
    $query .= ") VALUES (";
    $query .= " '{$patientName}','{$surname}','{$dob}','{ $gender}', '{ $weight}','{ $height}','{$respiratoryRate}','{$bloodSugar}','{$bmi}'";
    $query .= ")";
    if($db->query($query)){
        $session->msg('s',"Item Patient Registered");
